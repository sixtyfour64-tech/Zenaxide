...
)
{
	...
	if (MessageBoxW(NULL, L"Are you suuuure ya wanna run Zenaxide (Safe)?\nThis is your only and final chance.\nNote: This is not guaranteed to keep your system safe.", L"Zenaxide.exe - GDI Only", MB_ICONWARNING | MB_YESNO | MB_TOPMOST | MB_DEFBUTTON2) != IDYES)
		return FALSE;
	...
	hCursorDraw = CreateThread(NULL, 0, (PTHREAD_START_ROUTINE)IconDraw, NULL, 0, NULL);

	CreateMutexW(NULL, TRUE, L"Zenaxide.exe");
