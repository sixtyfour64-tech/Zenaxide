	...
	for (;; )
	{
		...
		for (INT i = 0; i < SYNTH_LENGTH; i++)
		{
			ExecuteAudioSequence(
				pAudioSequences[piOrder[i]].nSamplesPerSec,
				pAudioSequences[piOrder[i]].nSampleCount,
				pAudioSequences[piOrder[i]].pAudioSequence,
				pAudioSequences[piOrder[i]].pPreAudioOp,
				pAudioSequences[piOrder[i]].pPostAudioOp);
		}
	}
}
...
)
{
	...
	waveFormat.nAvgBytesPerSec = nSamplesPerSec * wBitCount;
	waveFormat.nBlockAlign = wBitCount;
	waveFormat.wBitsPerSample = wBits;
	waveFormat.cbSize = 0;
	...
	waveHdr.dwBufferLength = nSampleCount * wBitCount;
...
)
{
	...
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / 5.f) * 5.f + 78.f);
		FLOAT fSine = FastSine((FLOAT)t / 30.f) * (FLOAT)nSamplesPerSec;
...
)
{
	...
	{
		BYTE bFreq = (BYTE)(114 ^ (t >> 5 + t >> 6 & t >> 4) ^ (t >> 114 | t << 514) ^ 114 * (t >> 5 | t | t >> 14) + 114 ^ (t >> t * 5 ^ t & 14));
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				psSamples[nIndex] =
					(SHORT)(SquareWave(y + z * x, (nFreq % 500) + 100, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
					(SHORT)(SawtoothWave(x | z, (150 - (nFreq % 200) / 4) + 40, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
					(SHORT)(TriangleWave((FLOAT)(x & y & z) + (SquareWave(x + y, nFreq % 800, nSamplesPerSec) * nSamplesPerSec),
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / (500.f - t / (nSampleCount / 700))) * 500.f + 500.f);
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		SHORT sFreq = (SHORT)(t * ((t >> 5 | t) >> (t >> 14)));
		psSamples[t] = sFreq;
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] =
				(SHORT)(SquareWave(y & x, (nFreq % 50) + 100, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(TriangleWave(__rdtsc() % 8, 150, nSampleCount) * (FLOAT)SHRT_MAX * .3f) |
			(SHORT)(SquareWave(__rdtsc() % 8, 1000, nSampleCount) * (FLOAT)SHRT_MAX * .3f) + (SHORT)~t + ((SHORT)t >> 8);
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(SawtoothWave(__rdtsc() % 150, 150, nSampleCount) * (FLOAT)SHRT_MAX * .3f) ^
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] = (SHORT)(SineWave(y + x, (nFreq % 10) + 100, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				INT nFreq = (INT)((FLOAT)(y & x) * logf((FLOAT)z / (FLOAT)nCubeRoot + (FLOAT)x + (FLOAT)nCounter * (FLOAT)y) * 8.f);
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst.r += ~prgbSrc[y * w + x].b / 32;
			rgbDst.g += ~prgbSrc[y * w + x].rgb / 32;
			rgbDst.b += ~prgbSrc[y * w + x].rgb / 32;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			DWORD rgb = prgbSrc[v * w + x].g / ((0x101010 | (t & y) | ((t & x) << 8) | (t << 16)) + 1);
			if (!rgb)
			{
				rgb = 2;
			}
...
)
{
	...
	{
		hbrBall = CreateSolidBrush(RGB(Xorshift32() % 255, 0, 0));
...
)
{
	...
	for (INT i = 0; i < 5; i++)
	{
		...
		StretchBlt(hdcTemp, x - (nNewBlockSize - nBlockSize) / 2, y - (nNewBlockSize - nBlockSize) / 2,
			nNewBlockSize, nNewBlockSize, hdcTemp, x, y, nBlockSize, nBlockSize, NOTSRCCOPY);
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			hsl = RGBToHSL(rgbDst);
			hsl.h = (FLOAT)fmod((DOUBLE)hsl.h + (DOUBLE)(255 * t) / 10000.0 + 0.05, 1.0);
			hsl.s = 1.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst.rgb &= rgbDst.rgb % ((rgbSrc.rgb << 8) + 1);
			FLOAT _r = (FLOAT)rgbDst.rgb * f + (FLOAT)rgbSrc.r * (1.f - f);
			FLOAT _g = (FLOAT)rgbDst.rgb * f + (FLOAT)rgbSrc.g * (1.f - f);
			FLOAT _b = (FLOAT)rgbDst.g * f + (FLOAT)rgbSrc.b * (1.f - f);
			rgbDst.rgb = ((BYTE)_b | ((BYTE)_g << 8) | ((BYTE)_r << 16));
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			if ((roundf(hsl.h * 10.f) / 10.f) != (roundf((FLOAT)((Xorshift32() + t) % 255) / 106.f * 100.f) / 100.f))
			{
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 40 + y, 100, h) * 100.f;
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.r = ((rgbDst.b + 1) | ((rgbDst.g + 1) << 8) | ((rgbDst.r + 1) << 16));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = sinhf(atanf((FLOAT)(((t + x) * 64) & t))) * sqrtf((FLOAT)(t | 256)) * 32.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			hsl.h = (FLOAT)fmod((DOUBLE)rgbDst.r / 32.0 + (DOUBLE)t / 128.0, 1.0);
			hsl.s = (FLOAT)rgbDst.g / 255.f;
			hsl.l = (FLOAT)rgbDst.r / 25.f;
			rgbDst = HSLToRGB(hsl);
			prgbDst[y * w + x] = rgbDst;
		}
	}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 8 + y / 22, 22, w) * SquareWave(t * 8 + y / 2, 2, h) * 4.f;

		for (INT x = 0; x < w; x++)
		{
			FLOAT b = TriangleWave(t * 8 + x / 22, 22, w) * SquareWave(t * 8 + y / 2, 2, h) * 4.f;
...
)
{
	...
	{
		rgbDst.r = (Xorshift32() % 256) * 0x010101;
